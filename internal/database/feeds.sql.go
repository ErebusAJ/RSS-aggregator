...
func (q *Queries) DeleteFeed(ctx context.Context, arg DeleteFeedParams) (uuid.UUID, error) {
...
SELECT id, created_at, updated_at, title, url, user_id, last_fetched_at FROM feeds
`
...
const getNextFeedToFetchFrom = `-- name: GetNextFeedToFetchFrom :many
SELECT id, created_at, updated_at, title, url, user_id, last_fetched_at FROM feeds
ORDER BY last_fetched_at ASC NULLS FIRST
LIMIT $1
`

func (q *Queries) GetNextFeedToFetchFrom(ctx context.Context, limit int32) ([]Feed, error) {
	rows, err := q.db.QueryContext(ctx, getNextFeedToFetchFrom, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
